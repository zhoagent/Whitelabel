 ...
 *
 * @dependencies
 * - Relies on `src/types/env.d.ts` for `NodeJS.ProcessEnv` typings (implicitly via `process.env`).
 *
 * @returns
 ...
 * Throws an error if the environment variable is not set and is not one of the explicitly optional keys.
...
const getEnvVariable = (key: keyof NodeJS.ProcessEnv): string => {
  ...
  // Define keys that are explicitly optional
  const optionalKeys: Array<keyof NodeJS.ProcessEnv> = ['EXPO_PUBLIC_REVENUECAT_APPLE_KEY', 'EXPO_PUBLIC_REVENUECAT_GOOGLE_KEY'];
