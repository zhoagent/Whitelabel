/**
@fileoverview Initializes and exports the Supabase client instance, configured for React Native with AsyncStorage for session persistence, type-safe environment variables, and Supabase DB types.
...
@notes
react-native-url-polyfill must be imported globally (e.g., in `''@/app/_layout''`). This is correctly handled in the existing `src/lib/polyfills.ts` and its import in `src/app/_layout.tsx`.
...
Code is React Compiler friendly.
